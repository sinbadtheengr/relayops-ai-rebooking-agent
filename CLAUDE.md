...
npm start                                     # run the compiled app (dist/src/app.js)
npm test                                      # Vitest suite (hermetic in-memory db) — needs NO credentials
npm run mcp                                   # start the MCP server on stdio — needs NO credentials
```

Verify changes with `npm run build && npm test`, then `npm run seed && npm run scan` and a few `npm run demo:ask` queries. These CLI paths exercise db → scoring → relayops → agent without Slack. CI (`.github/workflows/ci.yml`) runs build + test on push/PR.
...
```
src/app.ts, src/scheduler.ts, src/mcp/*, scripts/*   adapters (Slack runtime, cron, MCP stdio, CLI)
        │  call
...
- `slackBlocks.ts` is pure `data → KnownBlock[]` functions (G-16 fixed) — callers fetch data and pass it in; keep it that way.
...
`REFERENCES` clauses are enforced — `getDb()` sets `PRAGMA foreign_keys = ON` (G-10 fixed); keep deletions child-first (`outreach_logs → appointments → customers`).
`outreach_logs` is read by `getRecentlyContactedCustomerIds` (G-01 fixed): a `contacted` log within `CONTACT_COOLDOWN_DAYS` (14, `src/relayops.ts`) suppresses the customer from opportunities and summaries unless `includeContacted: true` is passed.
...
1. Bounded tool loop (`MAX_TOOL_ROUNDS = 4`, G-06 fixed): every round passes `tools` + `tool_choice: "auto"`, so the model can chain calls (find opportunities → draft by id). When a round returns no tool calls, its content is the answer; if the budget is exhausted, one final completion without tools produces prose.
2. Tool args are zod-validated (G-07 fixed): `priority` coerced case-insensitively, `limit` clamped 1–25 (default 8), `minDays*` non-negative. Failures — including unknown `customerId` — become `{error}` tool *results*, never throws, so the model can recover.
3. Any throw anywhere → deterministic fallback, with the error logged.

**Tools** (`src/agent.ts`) — these are the AI's entire data surface, all read-only:
- `get_rebooking_opportunities(priority?, vipOnly?, minDaysSinceLastVisit?, minDaysOverdue?, serviceType?, includeContacted?, limit=8)` → `CustomerInsight[]`
- `draft_follow_up(customerId!, tone?)` → `{ customer, message }`
- `summarize_today()` → `DailySummary`
- `list_recently_contacted()` → `CustomerInsight[]` (suppressed by the contact cooldown)

**Deterministic fallback** (`parseDeterministicIntent`) — substring keyword routing, checked in this order: "contacted"/"suppressed" → recently-contacted list; summary/opportunities/recoverable revenue → daily summary text; "draft" → draft for top match; otherwise filtered list. Keywords: "high", "medium", "vip", "90", "today"/"contact". Loose matching is a known limitation (gap G-12). When adding a tool to the OpenAI path, add a corresponding keyword branch here — the fallback must stay at feature parity for credential-free demos.
...
- `mark_contacted` → `markCustomerContacted(id, note)` where the note records the acting Slack user (S-01 step 1) → inserts `outreach_logs` row (the draft text itself is still not persisted — gap G-08) → ephemeral confirmation. Failures answer ephemerally via `notifyStale` (G-09 fixed).
- `draft_customer` → ephemeral draft with its own *Mark contacted* button.

### 6.5 DMs, mentions, assistant threads — `src/app.ts:69-130`
- `app_mention`: strips `<@USERID>` tokens, adds 👀 reaction (guarded try/catch, G-03 fixed), sets assistant status if available, answers in-thread (`thread_ts` = existing thread or the mention's own ts). Empty mention text defaults to "Summarize today's opportunities".
...
7. **Slack event redelivery** — Slack retries events not acked within ~3s. The re-added 👀 (`already_reacted`) is now swallowed (G-03 fixed), but redelivered mentions still get answered twice. Handlers are not idempotent; keep them fast.
...
11. **Evening UTC skew** — fixed (G-13): `todayIso()` derives the date in `config.timezone` via `Intl.DateTimeFormat("en-CA")`, pinned by `test/utils.test.ts`. Timestamps in logs remain full UTC ISO strings by design.
...
- **Tests are hermetic**: `test/setup.ts` sets `DATABASE_URL=:memory:` and deletes `OPENAI_API_KEY` *before* config is imported; each test file gets its own worker/db. Keep new tests credential-free.
